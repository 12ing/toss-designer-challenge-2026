...
export function ParticipantConditionRow({
  ...
  const attendanceControl = row.attendanceLocked ? (
  ...
  return (
    ...
    >
      {/* Name / role (+ mobile context under role for non-organizer) */}
      <div className="flex min-w-0 items-start justify-between gap-3 min-[640px]:block">
        ...
        <div className="shrink-0 min-[640px]:hidden">{attendanceControl}</div>
      </div>

      {/* Shared schedule condition — desktop only */}
